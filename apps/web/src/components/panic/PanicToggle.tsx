...
/**
 * `<PanicToggle>` — header pill that flips Quiet Mode on/off.
 *
 * Phase 14 T2 (plan §2b). Factual label, *not* guilt-y:
 *   off → "Quiet mode" (offers the escape hatch)
 *   on  → "Quiet mode" (confirms you're in it)
 *
...
export function PanicToggle({ className }: Props) {
  ...
  return (
    <button
      ...
      aria-label={enabled ? "Exit quiet mode" : "Enter quiet mode"}
      className={cn(
