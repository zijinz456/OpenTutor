...
class MCPProvider:
    ...
    async def call_tool(self, name: str, arguments: dict) -> ToolResult:
        ...
        if not self._process or self._process.poll() is not None:
            return ToolResult(success=False, output="", error=f"MCP server '{self.server_name}' not running (crashed or stopped)")
        ...
        try:
            ...
            if self._process and self._process.poll() is not None:
                return ToolResult(success=False, output="", error=f"MCP server '{self.server_name}' crashed during tool execution")
