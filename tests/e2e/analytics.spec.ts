...
test.describe("Analytics Page", () => {
  ...
  test("shows loading spinner initially", async ({ page }) => {
    ...
    await expect(page.locator('svg.lucide-loader-2, .animate-spin').first()).toBeVisible();
  });
  ...
  test("shows 4 metric cards", async ({ page }) => {
    ...
    await expect(page.getByText("Diagnosed Errors")).toBeVisible();
  });

  test("shows 3 breakdown cards", async ({ page }) => {
    await page.goto("/analytics");
    await expect(page.getByText("Learning Analytics")).toBeVisible();
    await expect(page.getByText("Gap Types")).toBeVisible();
    await expect(page.getByText("Diagnoses")).toBeVisible();
